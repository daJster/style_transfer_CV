...
CONTENT_WEIGHT = 0.025 # 8e-4 # test different values
STYLE_WEIGHT = 1 # 8e-1, 8e-4 # test different values
TOTAL_VARIATION_WEIGHT = 8.5e-05   #8.5e-05
LEARNING_RATE = 12.0
...
def get_optimizer():
    return keras.optimizers.Adam(
        keras.optimizers.schedules.ExponentialDecay(
            initial_learning_rate = LEARNING_RATE, decay_steps = 445, decay_rate = 0.98 # test different values
...
def compute_loss(feature_extractor, combination_image, content_features, style_features):
    ...
    total_loss = (CONTENT_WEIGHT * loss_content +
                  STYLE_WEIGHT * loss_style +
                  TOTAL_VARIATION_WEIGHT * loss_total_variation)

    return total_loss, CONTENT_WEIGHT * loss_content, STYLE_WEIGHT * loss_style, TOTAL_VARIATION_WEIGHT * loss_total_variation
...
if __name__ == "__main__":
    ...
    style_image_path_2 = './dataset/psychedelic.jpg'
    ...
    # plot_features(style_features1["block1_conv1"], "block1_conv1")
    ...
    for iter in range(NUM_ITER):
        with tf.GradientTape() as tape:
            total_loss, loss_content, loss_style, loss_total_variation = compute_loss(
            model, generated_image, content_features, style_features1
        )

        grads = tape.gradient(total_loss, generated_image)
        optimizer.apply_gradients([(grads, generated_image)])

        # print("iter: %4d, loss: %8.f" % (iter, total_loss[0]))
        print("iter: ", iter,"losses:", total_loss.numpy())
